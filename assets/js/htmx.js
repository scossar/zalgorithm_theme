var htmx = (function () {
    ...
    while (parts.length > 0) {
      ...
      if (selector.indexOf("closest ") === 0) {
        ...
        item = scanForwardQuery(
          elt,
          normalizeSelector(selector.slice(5)),
          !!global,
        );
      } else if (
        selector === "previous" ||
        ...
        item = scanBackwardsQuery(
          elt,
          normalizeSelector(selector.slice(9)),
          !!global,
        );
  ...
  function shouldCancel(evt, elt) {
    if (evt.type === "submit" && elt.tagName === "FORM") {
      ...
      const btn = /** @type {HTMLButtonElement|HTMLInputElement|null} */ (
  ...
  function getTargetButton(target) {
    return /** @type {HTMLButtonElement|HTMLInputElement|null} */ (
        ...
        varsValues = maybeEval(
          ...
          function () {
            if (event) {
              ...
              // allow window.event to be accessible
  ...
  function verifyPath(elt, path, requestConfig) {
    ...
    const origin =
      location.protocol !== "about:" ? location.origin : window.origin;
